...
\item
  \begin{Ex}
    ...
    \begin{solution} \hfill \vspace{.75em} \\
      Chebyshev's inequality states that the probability that a random
      variable takes a value far from its mean is small.
      \begin{mdframed}[backgroundcolor=silver]
        \begin{description}
        \item[Chebyshev's Inequality] \hfill \vspace{.75em} \\
          $X$ is a RV with mean $\mu$ and variance $\sigma^2$
          \vspace{-1.0em}
          $$P(|Y-\mu_y| > d) \le \frac{\sigma^2}{d^2},
          \quad\forall\; d>0$$
        \end{description}
      \end{mdframed}
      ...
      \begin{equation}
        \label{eq:1-sol}
        \begin{aligned}
          P(|Y-\mu_Y|>d)&\le\frac{\sigma_Y^2}{d^2} \\
          &\le \frac{250000}{(2000)^2} \\
          \implies P(|Y-\mu_Y|>d) &\le 0.0625
        \end{aligned}
      \end{equation}
    \end{solution}
  \end{Ex}
